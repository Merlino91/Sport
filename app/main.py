...
app = FastAPI(title="EasySports Stremio Addon", version=ADDON_VERSION, lifespan=lifespan)
...
def extract_search_query(request: Request, extra: Optional[str] = None) -> Optional[str]:
    """Extracts and decodes search query from query params or extra path."""
    if hasattr(request, "query_params"):
        q_search = request.query_params.get("search")
    elif hasattr(request, "url") and hasattr(request.url, "params"):
        q_search = request.url.params.get("search")
    else:
        q_search = None

    if q_search:
        return urllib.parse.unquote(str(q_search)).strip()

    if not extra:
        return None

    unquoted = urllib.parse.unquote(extra)
    for part in unquoted.split("&"):
        if part.startswith("search="):
            query = part.split("search=", 1)[1].strip()
...
# Unconfigured Root Catalog Routes
@app.get("/catalog/{type}/{id}.json")
@app.get("/catalog/{type}/{id}/{extra}.json")
async def root_catalog_endpoint(request: Request, type: str, id: str, extra: Optional[str] = None):
    """Root catalog endpoint returning sports matches with UTC timezone."""
    base_url = get_base_url(request)
    search_query = extract_search_query(request, extra)
    items = await catalog_service.get_catalog(id, search=search_query, user_tz="UTC", base_url=base_url)
    return JSONResponse(content={"metas": items})


# Configured Catalog Routes
...
# Unconfigured Root Meta Route
@app.get("/meta/{type}/{id}.json")
async def root_meta_endpoint(request: Request, type: str, id: str):
    """Root meta detail endpoint."""
    base_url = get_base_url(request)
    meta = await catalog_service.get_meta_detail(id, user_tz="UTC", base_url=base_url)
    if not meta:
        raise HTTPException(status_code=404, detail="Meta not found")
    return JSONResponse(content={"meta": meta})


# Configured Meta Route
...
# Unconfigured Root Stream Route
@app.get("/stream/{type}/{id}.json")
async def root_stream_endpoint(type: str, id: str):
    """Root stream endpoint prompting user to configure their playback proxy."""
